...
class Classifier(Frame):        
    ...
    def positive(self):
        self.curr_tweet['physical_illness'] = 1.0
        #print self.curr_tweet['physical_illness']
        if 'id' in self.curr_tweet:
    ...
    def get_next_unclassified(self):
        while 'physical_illness' in self.curr_tweet:
            if not self.next_tweet():
                self.curr_tweet = {'text': "No tweets remaining."}
            elif int(self.curr_tweet['physical_illness']) == 0:
                return
            else:
